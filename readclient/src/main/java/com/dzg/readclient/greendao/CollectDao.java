package com.dzg.readclient.greendao;

import android.database.Cursor;
import android.database.sqlite.SQLiteStatement;

import org.greenrobot.greendao.AbstractDao;
import org.greenrobot.greendao.Property;
import org.greenrobot.greendao.internal.DaoConfig;
import org.greenrobot.greendao.database.Database;
import org.greenrobot.greendao.database.DatabaseStatement;

import com.dzg.readclient.mvp.model.Collect;

// THIS CODE IS GENERATED BY greenDAO, DO NOT EDIT.
/** 
 * DAO for table "COLLECT".
*/
public class CollectDao extends AbstractDao<Collect, Long> {

    public static final String TABLENAME = "COLLECT";

    /**
     * Properties of entity Collect.<br/>
     * Can be used for QueryBuilder and for referencing column names.
    */
    public static class Properties {
        public final static Property CreateAt = new Property(0, java.util.Date.class, "createAt", false, "CREATE_AT");
        public final static Property Id = new Property(1, Long.class, "id", true, "_id");
        public final static Property JokeId = new Property(2, int.class, "jokeId", false, "JOKE_ID");
        public final static Property UserId = new Property(3, int.class, "userId", false, "USER_ID");
        public final static Property IsUpload = new Property(4, int.class, "isUpload", false, "IS_UPLOAD");
        public final static Property JokeContent = new Property(5, String.class, "jokeContent", false, "JOKE_CONTENT");
    };


    public CollectDao(DaoConfig config) {
        super(config);
    }
    
    public CollectDao(DaoConfig config, DaoSession daoSession) {
        super(config, daoSession);
    }

    /** Creates the underlying database table. */
    public static void createTable(Database db, boolean ifNotExists) {
        String constraint = ifNotExists? "IF NOT EXISTS ": "";
        db.execSQL("CREATE TABLE " + constraint + "\"COLLECT\" (" + //
                "\"CREATE_AT\" INTEGER," + // 0: createAt
                "\"_id\" INTEGER PRIMARY KEY AUTOINCREMENT ," + // 1: id
                "\"JOKE_ID\" INTEGER NOT NULL ," + // 2: jokeId
                "\"USER_ID\" INTEGER NOT NULL ," + // 3: userId
                "\"IS_UPLOAD\" INTEGER NOT NULL ," + // 4: isUpload
                "\"JOKE_CONTENT\" TEXT);"); // 5: jokeContent
    }

    /** Drops the underlying database table. */
    public static void dropTable(Database db, boolean ifExists) {
        String sql = "DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "\"COLLECT\"";
        db.execSQL(sql);
    }

    @Override
    protected final void bindValues(DatabaseStatement stmt, Collect entity) {
        stmt.clearBindings();
 
        java.util.Date createAt = entity.getCreateAt();
        if (createAt != null) {
            stmt.bindLong(1, createAt.getTime());
        }
 
        Long id = entity.getId();
        if (id != null) {
            stmt.bindLong(2, id);
        }
        stmt.bindLong(3, entity.getJokeId());
        stmt.bindLong(4, entity.getUserId());
        stmt.bindLong(5, entity.getIsUpload());
 
        String jokeContent = entity.getJokeContent();
        if (jokeContent != null) {
            stmt.bindString(6, jokeContent);
        }
    }

    @Override
    protected final void bindValues(SQLiteStatement stmt, Collect entity) {
        stmt.clearBindings();
 
        java.util.Date createAt = entity.getCreateAt();
        if (createAt != null) {
            stmt.bindLong(1, createAt.getTime());
        }
 
        Long id = entity.getId();
        if (id != null) {
            stmt.bindLong(2, id);
        }
        stmt.bindLong(3, entity.getJokeId());
        stmt.bindLong(4, entity.getUserId());
        stmt.bindLong(5, entity.getIsUpload());
 
        String jokeContent = entity.getJokeContent();
        if (jokeContent != null) {
            stmt.bindString(6, jokeContent);
        }
    }

    @Override
    public Long readKey(Cursor cursor, int offset) {
        return cursor.isNull(offset + 1) ? null : cursor.getLong(offset + 1);
    }    

    @Override
    public Collect readEntity(Cursor cursor, int offset) {
        Collect entity = new Collect( //
            cursor.isNull(offset + 0) ? null : new java.util.Date(cursor.getLong(offset + 0)), // createAt
            cursor.isNull(offset + 1) ? null : cursor.getLong(offset + 1), // id
            cursor.getInt(offset + 2), // jokeId
            cursor.getInt(offset + 3), // userId
            cursor.getInt(offset + 4), // isUpload
            cursor.isNull(offset + 5) ? null : cursor.getString(offset + 5) // jokeContent
        );
        return entity;
    }
     
    @Override
    public void readEntity(Cursor cursor, Collect entity, int offset) {
        entity.setCreateAt(cursor.isNull(offset + 0) ? null : new java.util.Date(cursor.getLong(offset + 0)));
        entity.setId(cursor.isNull(offset + 1) ? null : cursor.getLong(offset + 1));
        entity.setJokeId(cursor.getInt(offset + 2));
        entity.setUserId(cursor.getInt(offset + 3));
        entity.setIsUpload(cursor.getInt(offset + 4));
        entity.setJokeContent(cursor.isNull(offset + 5) ? null : cursor.getString(offset + 5));
     }
    
    @Override
    protected final Long updateKeyAfterInsert(Collect entity, long rowId) {
        entity.setId(rowId);
        return rowId;
    }
    
    @Override
    public Long getKey(Collect entity) {
        if(entity != null) {
            return entity.getId();
        } else {
            return null;
        }
    }

    @Override
    protected final boolean isEntityUpdateable() {
        return true;
    }
    
}
